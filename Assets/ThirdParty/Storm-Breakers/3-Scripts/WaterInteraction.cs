...
namespace StormBreakers
{
    public class WaterInteraction : MonoBehaviour
    {
        ...
        private bool physicsInitialized;
        ...
        bool InitializePhysic()
        {
            ...
            {
                MeshCollider meshCollider = ResolveSimulationMeshCollider();
                if (meshCollider != null && meshCollider.sharedMesh != null)
                {
                    simulationMeshCollider = meshCollider;
                    simulatedMesh = meshCollider.sharedMesh;
                }
                else { Debug.LogError("Please add a hull generator component or mesh collider to this game object."); enabled = false; return false; }
            }

            // last check to be sure
            if (simulatedMesh == null) { Debug.LogError("No simulated mesh found. Please verify the existence of a hull generator component or mesh collider."); enabled = false; return false; }

            // getting and checking the rigidbody
            rb = GetComponent<Rigidbody>();
            if (rb == null)
            {
                Debug.LogError("Water Interaction requires a Rigidbody on the same GameObject.");
                enabled = false;
                return false;
            }
            ...
            if (!TryGetSimulationMeshData(simulatedMesh, out Vector3[] vertices, out int[] triangles))
            {
                enabled = false;
                return false;
            }

            rb.mass = Mathf.Max(0.001f, ComputeSimulationMeshVolume(vertices, triangles)*1000f*relativeDensity);
        ...
        private Vector3 GetSimulationVertexInInteractionSpace(Vector3 vertex)
        {
            if (simulationMeshCollider != null && simulationMeshCollider.transform != transform)
            {
                return transform.InverseTransformPoint(simulationMeshCollider.transform.TransformPoint(vertex));
            }

            return vertex;
        }

        private float ComputeSimulationMeshVolume(Vector3[] vertices, int[] triangles)
        {
            double signedVolume = 0d;

            for (int i = 0; i < triangles.Length; i += 3)
            {
                Vector3 vertex1 = Vector3.Scale(transform.localScale, GetSimulationVertexInInteractionSpace(vertices[triangles[i]]));
                Vector3 vertex2 = Vector3.Scale(transform.localScale, GetSimulationVertexInInteractionSpace(vertices[triangles[i + 1]]));
                Vector3 vertex3 = Vector3.Scale(transform.localScale, GetSimulationVertexInInteractionSpace(vertices[triangles[i + 2]]));
        ...
        private MeshCollider ResolveSimulationMeshCollider()
        {
            if (simulationMeshCollider != null)
            {
                return simulationMeshCollider;
            }
        ...
        private bool TryGetSimulationMeshData(Mesh mesh, out Vector3[] vertices, out int[] triangles)
        {
            vertices = null;
            triangles = null;
            ...
            {
                Debug.LogError("Water Interaction cannot read simulation mesh '" + mesh.name + "'. Enable Read/Write on the mesh import settings or assign a readable simulation mesh. " + exception.Message, this);
                return false;
            }

            if (vertices == null || vertices.Length == 0 || triangles == null || triangles.Length < 3)
            {
                Debug.LogError("Water Interaction simulation mesh '" + mesh.name + "' has no usable triangle data.", this);
                return false;
            }

            int usableTriangleIndexCount = triangles.Length - triangles.Length % 3;
            for (int index = 0; index < usableTriangleIndexCount; index++)
            {
                int vertexIndex = triangles[index];
                if (vertexIndex < 0 || vertexIndex >= vertices.Length)
                {
                    Debug.LogError("Water Interaction simulation mesh '" + mesh.name + "' contains triangle index " + vertexIndex + ", but only " + vertices.Length + " vertices are available.", this);
                    return false;
                }
            }
